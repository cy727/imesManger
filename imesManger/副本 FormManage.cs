...
namespace imesManger
{
    ...
    public partial class FormManage : Form
    {
        ...
        private void initDatatable(bool bPCode, bool bICode)
        {
            ...
            dtOrder.Columns.Add("Indentor Code", System.Type.GetType("System.String"));
            ...
            dtOrder.Columns.Add("Total Number", System.Type.GetType("System.Decimal"));
            dtOrder.Columns.Add("Manufacture Date", System.Type.GetType("System.String"));
            ...
            dtOrder.Columns.Add("Surplus", System.Type.GetType("System.Decimal")); //剩余 14
            ...
            for (i = 0; i < dSet.Tables["pi"].Rows.Count; i++)
            {
                ...
                oTemp[3] = dSet.Tables["pi"].Rows[i][7];
                oTemp[4] = dSet.Tables["pi"].Rows[i][3];
                ...
                var q2 = from dt1 in dSet.Tables["actual"].AsEnumerable()//查询最后
                         where (dt1.Field<string>("Product Code") == oTemp[1].ToString()) && (dt1.Field<string>("Indentor Code") == oTemp[5].ToString())//条件
                         select dt1;
                
                foreach (var item in q2)//显示查询结果
                {
                    //TAC
                    oTemp[6] = item.Field<int>("TAC ID");
                    oTemp[7] = item.Field<string>("TAC Code").ToString();

                    var qTAC = from dt2 in dSet.Tables["TAC"].AsEnumerable()//得到TAC初始值
                             where (dt2.Field<string>("Product Code") == oTemp[1].ToString()) && (dt2.Field<string>("Indentor Code") == oTemp[5].ToString()) && (dt2.Field<string>("TAC Code") == oTemp[7].ToString())//条件
                             select dt2;
                    foreach (var item1 in qTAC)//显示查询结果
                    {
                        oTemp[8] = item1.Field<int>("Init Number");
                        break;
                    }

                    //Manufacture
                    oTemp[9] = item.Field<int>("End Number");
                    oTemp[10] = item.Field<int>("Total Number");
                    oTemp[11] = item.Field<int>("Year").ToString() + "-" + item.Field<string>("Num of Week");
                    break;
                }
                ...
                {
                    oTemp[12] = item.Field<int>("End Number");
                    oTemp[13] = item.Field<int>("Year").ToString() + "-" + item.Field<string>("Num of Week");
                    break;
                }

                //剩余号码
                oTemp[14] = getSurplus(oTemp[1].ToString(), oTemp[5].ToString(), int.Parse(oTemp[9].ToString()));
                ...
                for (j = iNumofPass; j > 0; j--)
                {
                    ...
                    if (sT.Length == 2)
                    {
                        var q4 = from dt1 in dSet.Tables["orders"].AsEnumerable()//查询最后
                                 where (dt1.Field<string>("Product Code") == oTemp[1].ToString()) && (dt1.Field<string>("Indentor Code") == oTemp[5].ToString()) && (dt1.Field<int>("Year") == int.Parse(sT[0])) && (dt1.Field<string>("Num of Week") == sT[1])//条件
                                 select dt1;
            ...
            for (i = 0; i < dataGridViewP.RowCount; i++)
            {
                ...
                var q10 = from dt1 in dSet.Tables["orders"].AsEnumerable()//大于起始时间Order
                          where (dt1.Field<string>("Product Code") == dataGridViewP.Rows[i].Cells[1].Value.ToString()) && (dt1.Field<string>("Indentor Code") == dataGridViewP.Rows[i].Cells[5].Value.ToString()) && (dt1.Field<int>("Year") * 100 + int.Parse(dt1.Field<string>("Num of Week"))>=iTemp)
                         orderby (dt1.Field<int>("Year") * 100 + int.Parse(dt1.Field<string>("Num of Week")))  
                         select dt1;
                foreach (var item in q10)//显示查询结果
                {
                    cIme cime = new cIme(int.Parse(dataGridViewP.Rows[i].Cells[0].Value.ToString()),dataGridViewP.Rows[i].Cells[1].Value.ToString(), int.Parse(dataGridViewP.Rows[i].Cells[4].Value.ToString()),dataGridViewP.Rows[i].Cells[5].Value.ToString(), int.Parse(dataGridViewP.Rows[i].Cells[2].Value.ToString()));
                    ...
                    cime.dFailureRate = decimal.Parse(dataGridViewP.Rows[i].Cells[3].Value.ToString());
                ...
                for (j = ICOLUMNS; j < dataGridViewP.Columns.Count; j++)
                {
                    ...
                    var alQuery = from cIme cime in alIme
                                  where (cime.sPCode == dataGridViewP.Rows[i].Cells[1].Value.ToString()) && (cime.sICode == dataGridViewP.Rows[i].Cells[5].Value.ToString()) && (cime.iYear == int.Parse(sT1[0])) && (cime.iWeek == int.Parse(sT1[1]))
                                  select cime;
                    ...
                    {
                        cIme cime1 = new cIme(int.Parse(dataGridViewP.Rows[i].Cells[0].Value.ToString()), dataGridViewP.Rows[i].Cells[1].Value.ToString(), int.Parse(dataGridViewP.Rows[i].Cells[4].Value.ToString()), dataGridViewP.Rows[i].Cells[5].Value.ToString(), int.Parse(dataGridViewP.Rows[i].Cells[2].Value.ToString()));
                        cime1.iYear = int.Parse(sT1[0]);
                        cime1.iWeek = int.Parse(sT1[1]);

                        cime1.iCount = 0;
                        cime1.dFailureRate = decimal.Parse(dataGridViewP.Rows[i].Cells[3].Value.ToString());
                        alIme.Add(cime1);
                    }
                }
        ...
        private bool manageArrayList()
        {
            ...
            for (i = 0; i < dataGridViewP.RowCount; i++)
            {
                for (j = 10; j < dataGridViewP.ColumnCount; j++)
                {
                    string[] sT1 = dataGridViewP.Columns[j].Name.Split('-');
                    var alQuery = from cIme cime in alIme
                                  where (cime.sPCode == dataGridViewP.Rows[i].Cells[1].Value.ToString()) && (cime.sICode == dataGridViewP.Rows[i].Cells[4].Value.ToString()) && (cime.iYear == int.Parse(sT1[0])) && (cime.iWeek == int.Parse(sT1[1]))
                                  select cime;
                    foreach (cIme s in alQuery)
                    {
        ...
        private void changeViewText()
        {
            ...
            for (i = 0; i < dtOrder.Rows.Count; i++)
            {
                for (j = ICOLUMNS; j < dtOrder.Columns.Count; j++)
                {
                    ...
                    var alQuery = from cIme cime in alIme
                                  where (cime.sPCode == dtOrder.Rows[i][1].ToString()) && (cime.sICode == dtOrder.Rows[i][5].ToString()) && (cime.iYear == int.Parse(sT[0])) && (cime.iWeek == int.Parse(sT[1]))
                                  select cime;
                    if (alQuery.Count() <= 0)
                        continue;
                    foreach (cIme s in alQuery)
                    {
                        //最终日期
                        if (dtOrder.Rows[i][11].ToString() == (dtOrder.Columns[j].Caption))
                        {
        ...
        private void changeViewColor()
        {
            ...
            for (i = 0; i < dataGridViewP.Rows.Count; i++)
            {
                for (j = ICOLUMNS; j < dataGridViewP.Columns.Count; j++)
                {
                    ...
                    var alQuery = from cIme cime in alIme
                                  where (cime.sPCode == dataGridViewP.Rows[i].Cells[1].Value.ToString()) && (cime.sICode == dataGridViewP.Rows[i].Cells[5].Value.ToString()) && (cime.iYear == int.Parse(sT[0])) && (cime.iWeek == int.Parse(sT[1]))
                                  select cime;
                    if (alQuery.Count() <= 0)
        ...
        private void dataGridViewP_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string[] sT = dataGridViewP.Columns[e.ColumnIndex].Name.Split('-');
            if (dataGridViewP.Rows[e.RowIndex].Cells[11].Value.ToString() == dataGridViewP.Columns[e.ColumnIndex].Name) //最后实际阶段没有编辑
                return;
